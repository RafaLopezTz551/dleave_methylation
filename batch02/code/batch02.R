...
gids <- cl[[1]]$gene_id
stopifnot(sapply(cl, function(x) identical(x$gene_id, gids)))
...
fwrite(agg[, .(chr, win_start = as.integer(win), beta_pct = beta*100)],
...
region_beta <- function(rgr) {
  ...
  meth <- sum(ov$bg_c*ov$cv_c + ov$bg_a*ov$cv_a)
...
gb <- ov2[, .(beta = (sum(bg_c*cv_c)+sum(bg_a*cv_a))/pmax(sum(cv_c)+sum(cv_a),1), n_cpg = .N),
          by = .(gene_id, decile)][n_cpg >= 5 & !is.na(decile)]
...
ct <- cor.test(gbx$beta, gbx$expr_mean, method = "spearman", exact = FALSE)
...
DE_TAIL2 <- file.path(PIPE, "batch01/data/gene_de_tail.tsv")
de2 <- fread(DE_TAIL2)[, .(gene_id, log2FoldChange, padj)]
de2[, de_strict := !is.na(padj) & padj < 0.05 & abs(log2FoldChange) >= 1]
...
mw <- function(x, y) { w <- wilcox.test(x, y); list(P = w$p.value, rb = 2 * as.numeric(w$statistic) / (as.numeric(length(x)) * length(y)) - 1) }
...
if (file.exists(hifi_rds)) {
  ...
  hifi <- hifi[cov_1 >= 5 & cov_2 >= 5]
  saveRDS(hifi, hifi_rds)
}
...
stopifnot(sapply(bwl, function(x) identical(x$gene_id, bwl[[1]]$gene_id)))
...
p_mt <- ggplot(mt3, aes(sample, beta * 100, colour = grp)) +
  geom_jitter(position = position_jitter(width = 0.18, seed = 20260426), size = 0.8, alpha = 0.55) +
...
writeLines(capture.output(sessionInfo()), file.path(BATCH, "sessionInfo_batch02.txt"))
cat("[batch02] done\n")