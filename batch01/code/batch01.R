...
BATCH  <- "/mnt/data/alfredvar/rlopezt/meth_paper/main/methylation_pipeline/batch01"
...
save_supp <- function(p, name, w, h) {
  ...
  ggsave(file.path(SUPP, paste0(name, ".svg")), p, width = w, height = h)
...
di_counts <- colSums(dinucleotideFrequency(genome))
...
prom   <- trim(promoters(genes, upstream = 2000, downstream = 0))
...
region_chr_stat <- function(reg, chr) {
  ...
  strand(r) <- "*"; r <- reduce(trim(r)); r <- r[width(r) > 0]
...
gene_id <- sub(";.*", "", as.character(mcols(genes)$ID))
...
egg <- egg[locus %in% gene_id & !is.na(Preferred_name)]
...
broad_levels <- c("Writer","Eraser","Reader","Support","Cofactor",
                  "Demethylation pathway")
broad_cat <- function(x) sub(" *\\(.*", "", x)

tk <- copy(toolkit)
tk[, cat_broad := factor(broad_cat(as.character(category)), levels = broad_levels)]
tk[, label := ifelse(present, ortholog, gene_symbol)]
setorder(tk, -present, -bit, gene_symbol)
tk[, label := factor(make.unique(label), levels = rev(make.unique(label)))]
COL_TKCAT <- c(Writer = "#1B9E77", Eraser = "#D95F02", Reader = "#7570B3",
...
PFAM      <- file.path(PROJ, "tools/pfam/Pfam-A.hmm")
DNMT_Q    <- file.path(PROJ, "tools/dnmt/dnmt_queries.fasta")
...
best <- bl[order(evalue)][, .SD[1], by = qseqid]
...
cat_ids <- unique(sub("-mRNA.*", "", tbl_targets(cat_tbl)))
n_add   <- length(tbl_targets(add_tbl))

prot <- readAAStringSet(PROTE); names(prot) <- sub("\\s.*", "", names(prot))
pick <- unique(c(cat_ids, hit_loc))
sel  <- prot[names(prot) %in% paste0(pick, "-mRNA-1")]
...
count_mat <- count_mat[rowSums(count_mat >= 5) >= 2, , drop = FALSE]
...
tk_present <- toolkit[present == TRUE & gene_id %in% rownames(norm_mat)]
...
mrna <- data.table(gene_symbol = tk_present$ortholog,
...
lab_for <- function(ids) ifelse(is.na(sym_of[ids]), ids, sym_of[ids])
...
top_g <- head(deg[!is.na(padj)][order(padj)], 20)
setorder(top_g, log2FoldChange)
top_g[, dir := ifelse(log2FoldChange > 0, "Up", "Down")]
top_g[, dlab := disp_name(gene_id)]
...
molluscs <- data.table(
  ...
  fa = c(GENOME_FULL,
         file.path(MOLLDIR, "GCF_003073045.1_ASM307304v1_genomic.fna.gz"),
         file.path(MOLLDIR, "GCA_059052615.1_ASM5905261v1_genomic.fna.gz"),
         file.path(MOLLDIR, "GCF_000002075.1_AplCal3.0_genomic.fna.gz"),
         file.path(MOLLDIR, "GCF_001194135.2_ASM119413v2_genomic.fna.gz")))
...
local({
  ...
  PFAM_HMM  <- file.path(ROOT, "tools/pfam/Pfam-A.hmm")
  HMMER_BIN <- file.path(ROOT, "tools/hmmer/bin")
  PROTEOME  <- "/mnt/data/alfredvar/30-Genoma/31-Alternative_Annotation_EviAnn/derLaeGenome_namesDlasi_v2.fasta.functional_note.proteins.fasta"
  TAXGROUPS <- c("vertebrates", "insects", "nematodes", "urochordates")
  ...
  COL_A <- "#009E73"; COL_B <- "#E69F00"; COL_MARK <- "#333333"
  ...
  pd <- read_domtbl(need(PROT_DOM, "hmmsearch on proteome", min_rows = 1000L), 4, 1)
  ...
  gff <- readRDS(file.path(PIPE, "batch01/objects/gff_chrmt.rds"))
  ...
  cat("[TF 10] figures\n")
  unlink(list.files(FIGS, pattern = "^(figS2b_|figS_tf_)", full.names = TRUE))
  ...
  figE <- ggplot(wf) +
    ...
    labs(title = "How the D. laeve motif library is built",
         subtitle = "batch01, TF annotation. Each stage is one filter; a JASPAR motif reaches the library only if every answer is yes.") +
  ...
  cat("[batch01 TF-annotation section] done\n")
  
})

writeLines(capture.output(sessionInfo()), file.path(BATCH, "sessionInfo_batch01.txt"))
cat("[batch01] done\n")