...
rep_bp  <- sum(width(reduce(GRanges(te$chrom, IRanges(te$start, te$end)))))
...
per_te <- merge(te[, .(te_uid, chrom, start, end, te_name, class_family)], per_te, by = "te_uid")
per_te[, class := factor(class, levels = names(COL_CLASS))]
per_te[, loc := factor(loc, levels = names(COL_LOC))]
per_te[, beta_pooled := (beta_ctrl*1 + beta_amp*1)/2]
...
pb_dt <- copy(per_te)
pb_dt[, loc := factor(loc, levels = c("Intergenic", "Gene-overlapping"))]
...
qbrk <- c(-Inf, qhi[1:4], Inf)
qlab <- sprintf("Q%d", 1:5)
qrng <- sprintf("Q%d %.1f-%.1f%%", 1:5, qlo, qhi)
...
te_stats <- function(dt, beta_col, agg_tab, binned) {
  ...
  w  <- wilcox.test(g$beta, i$beta)
  rb <- 2 * as.numeric(w$statistic) / (as.numeric(nrow(g)) * nrow(i)) - 1
  ...
  out <- rbind(out,
               ...
               row("pct_kimura_gt_50", "All", 100 * mean(d$kimura > 50, na.rm = TRUE)))
  k <- d[!is.na(kimura)]
  ...
  q <- agg_tab[, .(value = beta[age_bin == "Q5"] - beta[age_bin == "Q1"],
                   monotone = as.numeric(all(diff(beta[order(as.character(age_bin))]) <= 0)),
...
writeLines(capture.output(sessionInfo()), file.path(BATCH, "sessionInfo_batch04.txt"))
cat("[batch04] done\n")