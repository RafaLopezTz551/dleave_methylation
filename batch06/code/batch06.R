...
B01 <- file.path(PIPE, "batch01/objects"); B01D <- file.path(PIPE, "batch01/data")
B02 <- file.path(PIPE, "batch02/objects"); B05D <- file.path(PIPE, "batch05/data")
BATCH <- file.path(PIPE, "batch06"); DAT <- file.path(BATCH, "data")
...
de <- if (file.exists(de_path)) fread(de_path) else data.table(gene_id = character(), log2FoldChange = numeric(), padj = numeric())

dmr_path <- file.path(B05D, "dmrs_annotated.tsv")
stopifnot(file.exists(dmr_path))                   
dmr_genes <- if (file.exists(dmr_path)) unique(fread(dmr_path)$gene_id) else character()
...
dmr_de <- merge(fread(dmr_path)[, .(gene_id, dMeth = -diff.Methy, region)],   
...
de[, g := sub(";.*", "", gene_id)]
sel_genes <- if (file.exists(isec_path)) unique(sub(";.*", "", fread(isec_path)$gene_id)) else character()
...
annot <- rbindlist(lapply(seq_len(nrow(sel)), function(i) {
  ...
  data.table(gene_id = r$g, symbol = disp(r$g), chr = r$chr, start = r$start, end = r$end,
             ...
             tss_in_host_intron = if (length(intron)) sum(overlapsAny(tss_gr, intron)) else 0L,
...
hdmr[, dMeth := -diff.Methy]            
hdmr[, ad := abs(dMeth)]; setorder(hdmr, -ad); hdmr[, ad := NULL]   
...
if (nrow(hdmr)) {
  ...
  pdf(file.path(FIGS, "figS6_hcp_promoter_dmrs.pdf"), width = 6.5, height = 5)   
  for (i in seq_len(nrow(hdmr))) draw_hcp(i)
  dev.off()
  for (i in seq_len(nrow(hdmr))) {                                             
...
writeLines(capture.output(sessionInfo()), file.path(BATCH, "sessionInfo_batch06.txt"))
cat("[batch06] done\n")
