#!/usr/bin/env Rscript
# 01_genome_toolkit.R
# Genomic CpG landscape of Deroceras laeve, the DNA-methylation toolkit it encodes and expresses,
# the CpG-depletion signature across molluscs, and the JASPAR-to-D. laeve transcription-factor bridge.
# Inputs: chr1-31 + mito genome FASTA, whole assembly, EviAnn GFF and proteome, TE table, HTSeq counts
#   (tail and tissue atlas), eggNOG-mapper annotations, Pfam-A, staged query proteins, four mollusc
#   assemblies, JASPAR2024 SQLite and Cis-BP thresholds. No earlier pipeline module is read.
# Outputs: data/    dinucleotide, region, toolkit, DE, domain, mollusc, tree and TF tables (.tsv, .nwk, .faa)
#          objects/ genome_chrmt.rds, gff_chrmt.rds, TF intermediates (seq/, diamond/, pfam/, trees/)
#          figures/ main fig1a-fig1e and fig2a; supplementary figS_* (each as .pdf, .png, .svg)
# Run: sbatch 01_genome_toolkit/code/01_genome_toolkit.slurm from main/methylation_pipeline/

# Step 1 - Seed, packages, paths, chromosome set, palette, theme and figure savers
...
# GENOME = chr1-31 + mito subset; GENOME_FULL = whole assembly, used only in Step 11
...
# Step 2 - Load genome and GFF (chr1-31 + mito), cached as objects/*_chrmt.rds
...
stale_cache <- function(cache, ...) !file.exists(cache) || any(file.mtime(c(...)) > file.mtime(cache))   # rebuild when any input is newer than the cache
if (!stale_cache(genome_rds, GENOME)) {
  ...
  genome <- genome[keep_chr]   # chromosome order chr1..chr31
...
# Set the GFF seqlevels to the 32-sequence universe on both paths (the mito scaffold has no features)
gff <- GenomeInfoDb::keepSeqlevels(gff, intersect(keep_chr, GenomeInfoDb::seqlevels(gff)),
                                   pruning.mode = "coarse")
GenomeInfoDb::seqlevels(gff) <- keep_chr
...
# Step 3 - fig1a: genome dinucleotide frequencies, observed vs expected
...
# Expected frequency = product of the two mononucleotide frequencies
...
# Step 4 - Per-region, per-chromosome CpG density, CpG O/E and GC content
cat("[3] per-region CpG density / O/E / GC\n")
genes <- gff[gff$type == "gene"]
exons <- gff[gff$type == "exon"]
seqlengths(genes) <- chr_len[seqlevels(genes)]

# Promoter = 2 kb upstream of the TSS; intergenic = genome minus gene bodies and promoters
prom   <- trim(promoters(genes, upstream = 2000, downstream = 0))  # 2 kb upstream
# Strand-neutralise before reduce() so opposite-strand overlaps merge
...
# Nuclear chromosomes only: the mito scaffold would enter the Intergenic series alone as a GC outlier
...
# Step 5 - fig1b/c/d: density of the per-chromosome region statistics
...
# Step 6 - fig2a: methylation toolkit presence from eggNOG-mapper orthology
cat("[5] fig2a toolkit presence\n")
# One row per gene family; vertebrate paralog sets are matched against every paralog name (tk_syn)
toolkit <- data.table(
...
# Present = best eggNOG hit at or above the mapper's default seed-ortholog bit score (60)
BIT_MIN <- 60
...
# Bar length = eggNOG bit score; label = the ortholog name assigned, or 'absent'
tk <- copy(toolkit)
...
# Step 7 - figS_dnmt3_absence: tBLASTn and Pfam evidence that DNMT3 is absent
...
# Step 7.1 - tBLASTn of DNMT query proteins vs the genome (DNMT1, DNMT2, TET3 as controls)
...
# The multi-word -outfmt must be one shell token (shQuote); system2 does not quote arguments
...
# Locate the gene under the best DNMT3 hit (expected: a PWWP-domain gene, not a methyltransferase)
...
# Step 7.2 - HMMER/Pfam: catalytic C5-MTase domains and DNMT3-specific ADD domains
# hmmfetch needs an SSI index; build it only if neither index name exists
...
sel  <- prot[names(prot) %in% paste0(pick, "-mRNA-1")]   # one isoform (-mRNA-1) per gene
...
# Step 7.3 - Panel A: best tBLASTn hit per query, annotated with the gene it lands in
...
# Step 7.4 - Panel B: domain architecture of the MTases, the BLAST hit and human DNMT3A
...
# Step 8 - figS_toolkit_mrna_tail: toolkit mRNA in tail, control vs amputated
cat("[6] fig2c toolkit mRNA, tail control vs amputated\n")
# Sample map from file names: C1S1-C4S4 control, T2S6-T4S8 amputated
samples <- data.table(
...
# Restrict to genes on the chromosome universe, then keep genes with >= 5 reads in >= 2 libraries
...
tk_present <- toolkit[present == TRUE & gene_id %in% rownames(norm_mat)]   # present and expressed
...
long <- data.table::melt(mrna, id.vars = c("gene_symbol","cat_broad"),
...
save_supp(pf, "figS_toolkit_mrna_tail", 5.0, 4.5)

# Step 9 - Tail gene-level differential expression (DESeq2 + apeglm) and DE figures
cat("[7] supplementary: gene DE\n")
suppressPackageStartupMessages({ library(EnhancedVolcano) })

# Gene symbols from the GFF 'Similar to SYM:' note; unnamed genes keep their locus id
...
# Step 9.1 - Wald test on the dds built in Step 8, apeglm shrinkage of the fold changes
dds <- DESeq(dds)
...
# Step 9.2 - Argonaute superfamily census: Pfam Piwi-domain loci and their tail expression
...
# PIWI clade when eggNOG names PIWIL* or the assembly symbol is Ago3/PIWI, otherwise AGO
...
# Step 9.3 - figS_gene_de_volcano: labels forced to the top 5 up and top 5 down named genes
...
# Duplicate symbols get their locus id appended so selectLab matches one point each
...
# Step 9.4 - figS_top_de_genes: the 20 most significant genes
...
# Step 10 - figS_uhrf_domains: UHRF and DNMT1 domain architecture against human UHRF1/UHRF2
...
if (file.exists(UHRF_REF) && file.exists(PROTEOME) && dir.exists(HMMER_BIN)) {
  ...
  stopifnot(length(uhrf_locus) == 1L, nzchar(uhrf_locus), length(dnmt1_locus) == 1L, nzchar(dnmt1_locus))   # loci from the eggNOG assignment in Step 6
...
# Pfam accessions of the domains relevant to maintenance methylation (tandem tudor = TTD, PF12148)
...
# Step 11 - fig1e: genome-wide CpG O/E across five mollusc assemblies
cat("[N] cross-species mollusc CpG O/E (deamination signature)\n")
MOLLDIR <- file.path(BATCH, "dataset", "mollusc_genomes")
# D. laeve enters as the whole assembly so every species is measured on the same kind of sequence
molluscs <- data.table(
...
# One genome loaded at a time; a per-species dinucleotide figure is written inside the loop
...
# Genus abbreviated on the axis; GC content printed under each bar
...
# Step 12 - figS_toolkit_mrna_atlas: toolkit expression across the 40-library tissue atlas
cat("[8b] toolkit mRNA across the 40-library atlas\n")
META <- "/mnt/data/alfredvar/rlopezt/WGCNA/metadata.tsv"
# Four expression outlier libraries are excluded; groups are never pooled across experiments
...
# Step 13 - figS_dnmt_c5_tree: ML tree of C5-methyltransferase (PF00145) domains
...
# Short tip ids: '|' and spaces in sequence names are fragile in Newick
...
# Best near-complete (>= 150 aa) DNA_methylase domain per protein
...
if (nrow(dt8) >= 4) {
# MAFFT -> trimAl -> IQ-TREE (ModelFinder, 1,000 ultrafast bootstraps, set seed), midpoint rooted
...
# -gappyout removes gap-rich columns only; sequences with > 50% gaps are then dropped
...
# Step 14 - TF annotation: JASPAR motifs to D. laeve orthologs (bridge and guide)
# Wrapped in local() so its helpers and constants do not collide with the sections above
local({
set.seed(20260426)
suppressPackageStartupMessages({
...
# Step 14.1 - Configuration and the two motif-library decisions (homology and identity rule)
...
PFAM_HMM  <- file.path(ROOT, "tools/pfam/Pfam-A.hmm")   # already hmmpress'ed
HMMER_BIN <- file.path(ROOT, "tools/hmmer/bin")   # HMMER 3.4
PROTEOME  <- "/mnt/data/alfredvar/30-Genoma/31-Alternative_Annotation_EviAnn/derLaeGenome_namesDlasi_v2.fasta.functional_note.proteins.fasta"
...
# ORTH_HOMOLOGY: 'rbh' or 'rbh_or_tree'; ORTH_IDENTITY: 'cisbp', 'w2014' or 'none'
ORTH_HOMOLOGY <- "rbh"
ORTH_IDENTITY <- "cisbp"
ORTH_DIMER    <- "any"   # "any": keep when >= 1 subunit passes; "both": all must pass
TREE_MAX_CLADE   <- 10L   # clades larger than this are uninformative
...
# Step 14.2 - Helpers: tool runner, output checks, atomic writes, domtbl parser, theme
...
# need(): a non-empty file is not enough; a minimum row count guards against truncated tables
...
fw <- function(x, path) {   # atomic write
...
# Step 14.3 - JASPAR2024 CORE matrices (four animal tax groups) to UniProt accessions
...
# Step 14.4 - Bait protein sequences from UniProt (staged once on a node with network access)
...
# A stale or truncated staged FASTA would shrink the whole library: > 10% missing baits stops the run
stopifnot(length(missing_acc) < 0.10 * length(accs))

# Fetch-only mode: stage the baits, check the thresholds file, then exit
...
# Step 14.5 - DIAMOND blastp both directions, isoforms collapsed to locus, RBH
...
# Reciprocal = best hit in both directions
...
# Step 14.6 - Pfam domains: hmmscan on baits, hmmsearch of DBD profiles on the proteome
...
# JASPAR structural class -> Pfam DBD family map
CLASS2PFAM <- list(
  ...
  "Nuclear receptors with C4 zinc fingers" = c("zf-C4"),   # Hormone_recep is the ligand-binding domain, not a DBD
...
# Validate DBD names against Pfam-A on disk first: hmmfetch aborts at the first missing name
...
# Step 14.7 - DBD licensing: bait and locus must both carry the class-expected domain
cat("[TF 5] DBD licensing: both proteins must carry the class-expected domain\n")
acc2class <- merge(m2a[nzchar(acc), .(matrix_id, acc)], mclass, by = "matrix_id", all.x = TRUE, allow.cartesian = TRUE)   # all.x keeps class-less matrices so NOCLASS2PFAM applies
...
# Step 14.8 - Pairwise DBD identity (MAFFT) on two denominators; the gate uses PctID_L
cat(sprintf("[6] pairwise DBD alignment for %d licensed pairs (MAFFT, %d cores)\n", nrow(lic), NPROC))
prot_rep <- prot_all[unique(lic$protein)]
# PctID_O = identities / aligned columns; PctID_L = identities / length of the longer domain
# MAFFT reads from stdin so forked workers never share a temporary file
pair_id <- function(i) {
...
# Align one pair first so a systematic failure surfaces with its error message
...
# Step 14.9 - Cis-BP / Weirauch per-family motif-transfer thresholds (staged file)
...
# Non-numeric Cis-BP cutoffs become NA and the cisbp gate excludes those families; the count is reported
...
# Step 14.10 - Per-family DBD gene trees (MAFFT, trimAl, IQ-TREE) and co-ortholog clades
cat("[TF 8] per-family DBD gene trees\n")
fam_tab <- lic[, .(n_baits = uniqueN(acc), n_loci = uniqueN(locus)), by = pfam][n_baits >= 2 & n_loci >= 1]
# Families with fewer than 4 sequences cannot be bootstrapped and get no tree
...
tree_rows <- rbindlist(lapply(fam_tab$pfam, function(f) {
  sub <- lic[pfam == f]
# Family FASTAs are cut to the Pfam DBD envelope, not full length
...
# Midpoint root first: the walk from a locus up to the nearest bait-containing clade needs a rooted tree
...
# Step 14.11 - Apply the gate and write the JASPAR-to-D. laeve bridge
...
# Step 14.12 - The D. laeve TF guide: one row per kept motif-locus pair with every filter value
cat("[TF 9b] the D. laeve TF guide\n")
# Coordinates from the chromosome-filtered GFF; loci off chr1-31 + mito stay with on_assembly = FALSE
...
# Companion FASTA with the full protein sequence of every kept D. laeve TF
...
# Step 14.13 - Funnel counts, per-family DBD conservation and the decision-grid tables
cat("[TF 10] figures\n")
unlink(list.files(FIGS, pattern = "^(figS2b_|figS_tf_)", full.names = TRUE))   # remove stale TF figure stems only
...
# Step 14.14 - figS_tf_tf_family_complement: loci per class-expected DBD family
...
# Absent families are drawn as open circles left of 1 on the log axis
...
# Step 14.15 - figS_tf_tf_annotation_workflow: the method as a diagram with the funnel counts
wf <- data.table(
  step  = 1:10,
  ...
  n     = c(fun$n[1], fun$n[2], fun$n[3], NA, fun$n[5], fun$n[4], fun$n[6], NA, fun$n[7], fun$n[8]))   # rows 5-7: candidate exists / bait has a Pfam domain / same class-expected domain
...
# Step 15 - Record the package versions used by this run
