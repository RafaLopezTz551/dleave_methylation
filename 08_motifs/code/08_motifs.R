...
save_plot(FIGM, "fig8_lmr_motif_enrichment",
          function() draw_motif_hm(label_ids(seSel), "LMR sequences — TF motifs across the methylation-change gradient",
                                   cluster = hcl, dendro = !isTRUE(hcl)), w = 8.6, h = hh * 0.8)   # legend titles clipped mid-word at w=7.6 ("log2 enric") fixed: canvas widened to 8.6 (figure QA 2026-08-28)
...
se_w_rds <- file.path(OBJ, "promoter_weber_pc_chrmt_se.rds")          # ⚠️ versioned on universe, NOT the PWM library — delete after a 01_genome_toolkit rebuild
...
%s</svg>', 30 + ncol * COLW, as.integer(TOP + nr * ROWH),
  ...
  run_homer(umr_prom, "umr_promoter", "TF motifs at unmethylated promoters",
            cpg = TRUE, main = TRUE)      # MAIN figure (author request 2026-07-25)
